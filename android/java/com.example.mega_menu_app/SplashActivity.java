...
public class SplashActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Glide.with(this).load(R.drawable.mega_menu_run_cat).into(gif_image);

        // AsyncTask를 사용하여 데이터를 받아오는 작업을 처리
        new SplashActivity.GetJsonData().execute("AWS API Gateway URL");
        ...
        protected void onPostExecute(String jsonData) {
            ...
            try {
                ...
                for (int i=0; i < data.length(); i++) {
                    ...
                    if (index != -1) {
                        String menu_date = menu.substring(0, index); // 첫번째 개행문자 앞쪽 문자열 추출
