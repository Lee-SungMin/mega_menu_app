...
public struct CarouselView<Content: View, Data: Hashable>: View {
    ...
    public init(items: [Data], spacing: CGFloat = 0, currentIndex: Binding<Int>, @ViewBuilder content: @escaping (Data) -> Content) {
        ...
        //self.offsetShift = itemWithSpacing * itemsCount / 2 + itemWithSpacing * 2 // (월요일)
        //self.offsetShift = itemWithSpacing * itemsCount / 2 + itemWithSpacing (화요일)
        //self.offsetShift = itemWithSpacing * itemsCount / 2 (수요일)
        //self.offsetShift = itemWithSpacing * itemsCount / 2 - itemWithSpacing (목요일)
        //self.offsetShift = itemWithSpacing * itemsCount / 2 - itemWithSpacing * 2 (금요일)
